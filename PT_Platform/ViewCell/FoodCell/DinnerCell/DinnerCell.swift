...
class DinnerCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        self.datalistDinner = Shared.shared.DinnerFoodMArray as! [DinnerFoodM]
        setupTableView()
...
extension DinnerCell : UITableViewDataSource, UITableViewDelegate {
     func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
         return datalistDinner.count
    }
    
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "SubFoodCell", for: indexPath) as? SubFoodCell
            cell?.lblTitle.text = datalistDinner[indexPath.row].title as? String ?? ""
            cell?.lblName.text = "calorie \(Int(datalistDinner[indexPath.row].calorie as? Double ?? 0.0))"
            cell?.selectionStyle = .none
            return cell!
    }
    
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let food = datalistDinner[indexPath.row]
