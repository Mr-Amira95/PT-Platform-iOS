...
class BreakfastCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        self.datalistBreakfast = Shared.shared.BreakfastFoodMArray as! [BreakfastFoodM]
        setupTableView()
...
extension BreakfastCell : UITableViewDataSource, UITableViewDelegate {
     func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
         return datalistBreakfast.count
    }
    
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "SubFoodCell", for: indexPath) as? SubFoodCell
            cell?.lblTitle.text = datalistBreakfast[indexPath.row].title as? String ?? ""
            cell?.lblName.text = "calorie \(Int(datalistBreakfast[indexPath.row].calorie as? Double ?? 0.0))"
            cell?.selectionStyle = .none
            return cell!
    }
    
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let food = datalistBreakfast[indexPath.row]
